...
npm install
npm run dev
(on another terminal) npm run dev:server
open http://localhost:5173 in your browser

● Prerequisites:
● Node.js v18+
● (Optional — for receipt scanner) A Google Cloud Vision API key. Create a .env file in the project root from the .env.example: GOOGLE_CLOUD_VISION_API_KEY=your_key_here
● Run Commands:
...
● Give one example of a suggestion you rejected or changed: Originally, I did not fully describe the functionality of how trashing an item would update insights, resulting in an incorrect implementation that didn't log the deletion as a UsageRecord. I corrected the prompt to specify that the trash button in Usage Mode must write a UsageRecord with source: 'expired_removal' to IndexedDB before removing the item, so the Insights page can accurately compute food waste totals and percentages.
...
● What would you build next if you had more time? Definitely would try to implement the planning page, because I feel like I would find it useful myself to figure out what quantities of food I actually need, and then having insights based on my usage to order a reasonable amount of each ingredient. I'd also like to add flesh out the search page, and have suggestions to order more of an ingredient that you are running out of from sustainable sources. 
● Known limitations:
● Planning page is a stub — the "New Meal Plan" button and all meal-plan logic are not implemented.
● Suppliers page uses hardcoded data; the "View Details" button is not wired up and distance is static.
● All inventory and usage data is stored client-side (localStorage + IndexedDB) — no cross-device sync.
